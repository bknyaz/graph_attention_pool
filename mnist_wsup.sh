params="--epochs 30 --lr_decay_step 20,25 --test_batch_size 200 -K 4 --aggregation mean -D mnist --n_hidden 0 --readout max -f 4,64,512 --dropout 0.5 --threads 0 --img_features mean,coord --img_noise_levels 0.5,0.75 --pool_arch fc_prev --kl_weight 100"
...
for i in $(seq 1 1 10);
  ...
  python main.py  $params --seed $seed --pool attn_sup_threshold_skip_skip_"$thresh" --alpha_ws $results_dir/global_max/alpha_WS_train_seed"$seed".pkl --results $results_dir/wsup | tee $results_dir/wsup/wsup_seed"$seed".log;
done
